...
def GenomicRangeQuery(S, P, Q):         # Correctness 100%, performance 0%,  time complexity: O(N * M)
    Gen = { 'A':1, 'C':2, 'G':3, 'T':4 }
    M = len(P)
    result = [-1]*M
...
def Verify():
    AA = [ ["CAGCCTA", [2,5,0], [4,5,6] ],
          ["ATCGCCAAGACTTACGT", [3,0,7,4,2], [8,3,11,9,7]]
        ]
    for a in AA:
        ret = GenomicRangeQuery(a[0],a[1],a[2])
        print("%s --> %s" % (a, ret))
